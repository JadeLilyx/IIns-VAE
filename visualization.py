...
def visualize_latents_encircle(features_arr, labels_arr, circ_labels_arr, save_path, epoch,
                               dataset_name='zenodo', dataset_env='obstacle_full', level='room',
                               method="umap", n_components=2, encircle_flag=True, title=None):
    # match labels and features to lists
    data_module = dict()
    ...
    # data_module for scatter points with labels_arr
    classes = set()
    for i in range(len(labels_list)):
        label_item = labels_list[i][0]
        if label_item not in data_module:
    ...
    # circ_data_module for encircling with cir_labels_arr
    circ_classes = set()
    ...
    if n_components == 2:
        fig = plt.figure()
        ax = fig.add_subplot()
        # plot scatter points with data_module
        for cls, color in zip(classes, plt.cm.get_cmap('tab20').colors):
            # print("int label: %d" % cls)
            class_features = np.asarray(data_module[cls])
            label_str_dict = label_dictionary(dataset_name, dataset_env, level)
            ...
            plt.legend([label_str_dict[item] for item in label_str_dict],
                       loc="upper right", title="Environment Labels")
        # encircle data points with circ_data_module
        if encircle_flag:
            for cls2, color2 in zip(circ_classes, plt.cm.get_cmap('tab20').colors):
                # print("int label2: %d" % cls2)
                ...
                # encircle(circ_class_features[:, 0], circ_class_features[:, 1], ec='firebrick', fc='none', linewidth=1.0)
...
def visualize_data(result_path, data_test, method="umap", n_components=2,
                   dataset_name="ewine", dataset_env="nlos", encircle_flag=True):
    if dataset_name == 'ewine':
        ...
        visualize_latents_encircle(features_arr, label_arr, label_arr,  # circ_label_arr=label_arr
                                   result_path, epoch=00, dataset_name=dataset_name,
                                   method=method, n_components=n_components, encircle_flag=encircle_flag)
        ...
        #                   method=method, n_components=n_components)
        visualize_latents_encircle(features_arr, rlabel_arr, rlabel_arr,  # better be rlabel_rough
                                   result_path, epoch=00, dataset_name=dataset_name, dataset_env=dataset_env,
                                   level="room", method=method, n_components=n_components, encircle_flag=encircle_flag)
        visualize_latents_encircle(features_arr, olabel_arr, olabel_arr,  # better be olabel_rough
                                   result_path, epoch=00, dataset_name=dataset_name, dataset_env=dataset_env,
                                   level="obstacle", method=method, n_components=n_components, encircle_flag=encircle_flag)
        # if successful transfer 'rlabel_arr' to an additional rough 'olabel_arr'
        visualize_latents_encircle(features_arr, olabel_arr, rlabel_arr,  # better be rlabel_rough
                               result_path, epoch=00, dataset_name=dataset_name, dataset_env=dataset_env,
                               level="obstacle", method=method, n_components=n_components, encircle_flag=encircle_flag)
...
if __name__ == '__main__':
    ...
    result_path = "saved_results/data_%s-%s/visual_data/" % (opt.dataset_name, opt.dataset_env)
