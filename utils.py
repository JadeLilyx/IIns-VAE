...
def get_args(parser):
    ...
    parser.add_argument("--dataset_env", type=str, default='room_full',
                        help="dataset (zenodo) of different environments, including nlos, rooms, and obstacles")
...
def reduce_latents(env_latent, labels):
    ...
    return reduced_latents, labels  # (n, 2), (n, 1)


def visualize_latents(latents_arr, labels_arr, dataset_env='nlos'):  # save_path, epoch
    
    data_module = dict()
    labels_list = labels_arr.tolist()
    reduced_latents_list = latents_arr.tolist()
